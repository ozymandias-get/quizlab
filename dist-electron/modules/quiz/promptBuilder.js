 ...
 */
function sanitizeUserInput(input, maxLength = 150) {
 ...
 */
function sanitizeStringArray(arr, maxItems = 10, maxLength = 100) {
 ...
 */
function buildQuizPrompt(params, pdfPath, outputFilePath) {
    const { questionCount = 5, difficulty = 'MEDIUM', style = ['MIXED'], focusTopic = '', language = 'tr', remedialTopics = [], // For remedial mode
    ...
    const safeLanguage = allowedLanguages.includes(language) ? language : 'tr';
    ...
    - "Cevabı bilmeyen bir öğrenci, şıklardan eleyerek doğruyu bulabilir mi? (Bunu engelle)"`,
            ...
            focus: (topic) => topic ? `ODAK KONUSU: "${topic}"` : "GENEL KAPSAM",
            styles: {
    ...
    - "Can a student guess the answer by elimination? (Prevent this)"`,
            ...
            focus: (topic) => topic ? `FOCUS TOPIC: "${topic}"` : "GENERAL SCOPE",
            styles: {
    ...
    if (style.includes('MIXED')) {
        ...
        const validStyles = style
            ...
            .filter(s => s in medicalStyleMapping)
            .map(s => `- ${medicalStyleMapping[s]}`)
            .join('\n');
    ...
    return `
    COMMAND: Analyze the file @${path_1.default.basename(pdfPath)} and generate a ${safeQuestionCount}-question quiz.
