...
async function getLatestRelease() {
    const { OWNER, REPO } = constants_1.APP_CONFIG.GITHUB;
    ...
    try {
        ...
        const response = await (0, cross_fetch_1.default)(url, {
            headers: {
                'User-Agent': `Electron-App/${electron_1.app.getVersion()}`, // Generic UA to avoid GitHub blocking
        ...
        if (message && (message.includes('fetch') || message.includes('network'))) {
            return new Promise((resolve) => {
                ...
                request.setHeader('User-Agent', `Electron-App/${electron_1.app.getVersion()}`);
...
function initUpdater() {
    ...
    electron_1.ipcMain.handle(IPC_CHANNELS.CHECK_FOR_UPDATES, async () => {
        if (isChecking)
            return { available: !!updateInfo, cached: true };
        const now = Date.now();
    ...
    electron_1.ipcMain.handle(IPC_CHANNELS.OPEN_RELEASES, async () => {
        const { OWNER, REPO } = constants_1.APP_CONFIG.GITHUB;
        const url = updateInfo?.htmlUrl || `https://github.com/${OWNER}/${REPO}/releases`;
        await electron_1.shell.openExternal(url);
    });
    electron_1.ipcMain.handle(IPC_CHANNELS.GET_APP_VERSION, () => electron_1.app.getVersion());
}
