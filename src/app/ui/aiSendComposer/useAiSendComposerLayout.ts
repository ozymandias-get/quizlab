...
function clampLayout(layout: DockLayout, panelHeight = layout.height): DockLayout {
    ...
    const width = clamp(layout.width, MIN_WIDTH, Math.min(MAX_WIDTH, window.innerWidth - VIEWPORT_PADDING * 2))
    const height = clamp(layout.height, MIN_HEIGHT, Math.min(MAX_HEIGHT, window.innerHeight - VIEWPORT_PADDING * 2))
...
export function useAiSendComposerLayout(itemsLength: number) {
    ...
    const handleResizeMove = useCallback((event: React.PointerEvent<HTMLButtonElement>) => {
        ...
        const width = resizeStateRef.current.startWidth + (event.clientX - resizeStateRef.current.startX)
        const height = resizeStateRef.current.startHeight + (event.clientY - resizeStateRef.current.startY)
