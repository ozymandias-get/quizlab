...
export const useBottomBarStyles = (
    ...
    () =>
      ({
        ...
        transition: performanceMode
          ? 'none'
          : 'width 0.4s cubic-bezier(0.16, 1, 0.3, 1), flex-basis 0.4s cubic-bezier(0.16, 1, 0.3, 1)'
      }) as CSSProperties,
    [clampedOpacity, clampedScale, scaledBaseSize, performanceMode]
  )
  ...
  const stackStyle = useMemo<CSSProperties>(
    () => ({
      ...
      willChange: 'auto',
      transition: performanceMode ? 'none' : 'width 0.4s cubic-bezier(0.16, 1, 0.3, 1)'
    }),
    [scaledBaseSize, performanceMode]
  )
  ...
  const panelStyle = useMemo<CSSProperties>(
    () => ({
      ...
      backdropFilter: performanceMode ? 'none' : 'blur(16px) saturate(118%)',
      WebkitBackdropFilter: performanceMode ? 'none' : 'blur(16px) saturate(118%)',
      border: `1px solid rgba(255, 255, 255, ${performanceMode ? 0.05 : 0.06 + clampedOpacity * 0.06})`,
      ...
      transition: performanceMode
        ? 'none'
        : 'background 0.5s cubic-bezier(0.16, 1, 0.3, 1), border-color 0.5s cubic-bezier(0.16, 1, 0.3, 1), border-radius 0.5s cubic-bezier(0.16, 1, 0.3, 1), box-shadow 0.5s cubic-bezier(0.16, 1, 0.3, 1)'
    }),
    [clampedOpacity, clampedScale, performanceMode]
  )
    ...
    () =>
      ({
        ...
        background: isOpen
          ? performanceMode
            ? '#1e1b18'
            : HUB_VISUAL_TOKENS.openBackground
          : performanceMode
            ? '#12100e'
            : HUB_VISUAL_TOKENS.closedBackground,
        boxShadow: isOpen
          ? performanceMode
            ? '0 2px 8px rgba(136, 102, 58, 0.3)'
            : HUB_VISUAL_TOKENS.openShadow
          : performanceMode
            ? '0 2px 8px rgba(0, 0, 0, 0.4)'
            : HUB_VISUAL_TOKENS.closedShadow,
        ...
        transition: performanceMode
          ? 'none'
          : 'background 0.5s cubic-bezier(0.16, 1, 0.3, 1), box-shadow 0.5s cubic-bezier(0.16, 1, 0.3, 1), border-radius 0.5s cubic-bezier(0.16, 1, 0.3, 1)'
      }) as CSSProperties,
    [isOpen, clampedOpacity, scaledBaseSize, clampedScale, performanceMode]
  )
