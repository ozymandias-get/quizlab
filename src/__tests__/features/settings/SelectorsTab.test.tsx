...
vi.mock('@ui/components/Icons', () => ({
    ...
    ExternalLinkIcon: ({ className }: { className?: string }) => <span className={className}>ExternalLinkIcon</span>,
...
describe('SelectorsTab', () => {
    ...
    it('renders advanced selector controls and validates the current tab', async () => {
        ...
        await waitFor(() => {
            expect(mockGenerateValidateSelectorsScript).toHaveBeenCalledWith(expect.objectContaining({
                input: '#prompt',
                button: '#send'
            }))
        })
    ...
    it('opens the requested AI tab for re-pick and can reset selectors', async () => {
        ...
        fireEvent.click(screen.getByRole('button', { name: /ChatGPT/i }))
        fireEvent.click(screen.getAllByRole('button', { name: /selectors_open_repick/ })[0] as HTMLElement)
