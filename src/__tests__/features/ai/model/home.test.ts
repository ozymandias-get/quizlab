...
import { getEnabledAiIdsByType, getFeaturedAiIds, mergeOrderedIds, safeAiAccentColor } from '@features/ai/model/home'
...
describe('ai home model helpers', () => {
    ...
    it('filters enabled ids by site type and builds featured ids', () => {
        ...
        expect(getEnabledAiIdsByType(['modelA', 'siteA', 'modelB'], aiSites, 'model')).toEqual(['modelA', 'modelB'])
        expect(getEnabledAiIdsByType(['modelA', 'siteA', 'siteB'], aiSites, 'site')).toEqual(['siteA', 'siteB'])
        expect(getFeaturedAiIds(['m1', 'm2', 'm3', 'm4'], ['s1', 's2', 's3'])).toEqual(['m1', 'm2', 'm3', 's1', 's2'])
    })
})
