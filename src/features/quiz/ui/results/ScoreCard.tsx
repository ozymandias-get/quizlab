...
export const ScoreCard: React.FC<ScoreCardProps> = ({ stats, settings, t }) => {
  ...
  const getGradeInfo = () => {
    if (stats.percentage >= 90)
      return { text: t('quiz_grade_perfect'), className: 'bg-emerald-500/20 text-emerald-400' }
    if (stats.percentage >= 70)
      return { text: t('quiz_grade_good'), className: 'bg-green-500/20 text-green-400' }
    if (stats.percentage >= 50)
      return { text: t('quiz_grade_average'), className: 'bg-amber-500/20 text-amber-400' }
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col md:flex-row items-center gap-6">
        ...
        <ConfettiCanvas isActive={stats.percentage >= 70} className="absolute inset-0" />

        <div className="flex-1 text-center md:text-left">
          <div
            className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-bold mb-2 ${grade.className}`}
          >
