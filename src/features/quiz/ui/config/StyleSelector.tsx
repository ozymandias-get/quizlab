            ...
            <motion.button
              ...
              onClick={() => {
                setSettings((s) => {
                  ...
                  let newStyles: QuestionStyleEnum[] = s.style.filter(
                    (x) => x !== QuestionStyle.MIXED
                  )
              ...
              whileTap={{ scale: 0.98 }}
              className={cn(buttonBaseClass, 'quiz-option-btn', isSelected ? 'selected' : '')}
            >
              <IconComp
                className={`w-4 h-4 shrink-0 ${isSelected ? 'text-amber-400' : 'text-white/40'}`}
              />
              <span
                className={`text-xs font-bold truncate ${isSelected ? 'text-amber-300' : 'text-white/50'}`}
              >
                {t(styleKey)}
              </span>
            </motion.button>
