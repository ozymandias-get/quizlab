        ...
        {models.map((m) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className={cn(buttonBaseClass, 'quiz-model-btn', isSelected ? 'selected' : '')}
            >
              <div
                className={`p-2 rounded-lg transition-all ${
                  isSelected ? `bg-gradient-to-br ${m.color} shadow-lg` : 'bg-white/10'
                }`}
              >
                <IconComp className={`w-5 h-5 ${isSelected ? 'text-white' : 'text-white/50'}`} />
              </div>
              <span className={`text-xs font-bold ${isSelected ? 'text-white' : 'text-white/60'}`}>
                {m.label}
              </span>
              <span className={`text-[10px] ${isSelected ? 'text-amber-400/80' : 'text-white/30'}`}>
                {m.desc}
              </span>
            </motion.button>
          )
