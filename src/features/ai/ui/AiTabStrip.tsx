...
const getVisibleTabIds = (tabs: Tab[], activeTabId: string): Set<string> => {
    if (tabs.length <= 3) {
...
function AiTabStrip() {
    ...
    const getTabLabel = useCallback((tab: Tab) => {
    ...
    const getTabColor = useCallback((tab: Tab) => {
                    ...
                    return (
                        ...
                        >
                            <span className="text-white/90 shrink-0">
                                {getAiIcon(aiSites[tab.modelId]?.icon || tab.modelId) || (
                            ...
                            <div className="absolute right-1.5 top-1/2 -translate-y-1/2 z-10 flex items-center gap-1">
                                {(tab.pinned || isHovered) && (
                                    <span
                                        ...
                                        className={`flex items-center justify-center rounded-md border p-1 transition-colors ${tab.pinned
