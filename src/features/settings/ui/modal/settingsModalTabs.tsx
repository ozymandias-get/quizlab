...
import { SettingsIcon, LanguageIcon, InfoIcon, GridIcon, EyeIcon, MagicWandIcon, SelectorIcon, TerminalIcon, GeminiIcon } from '@ui/components/Icons'
...
export const SETTINGS_TAB_RENDERERS: Record<SettingsTabId, (context: SettingsContext) => React.ReactNode> = {
    prompts: () => <PromptsTab />,
    models: () => <ModelsTab />,
    sites: () => <SitesTab />,
...
export function buildSettingsTabDefs(t: (key: string) => string): TabDef[] {
    return SETTINGS_TABS.map((tab) => ({
        id: tab.id,
        label: t(tab.labelKey) || ('fallbackLabel' in tab ? tab.fallbackLabel : undefined) || tab.labelKey,
