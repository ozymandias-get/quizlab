...
const ACTION_BUTTON_CLASSNAMES: Record<GeminiCliActionButtonVariant, string> = {
  primary:
    'inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-bold text-gray-800 shadow-lg transition-all hover:bg-gray-100 disabled:opacity-50',
  danger:
    'inline-flex items-center gap-2 rounded-xl border border-red-500/30 bg-red-500/20 px-4 py-2 text-sm font-bold text-red-400 transition-all hover:bg-red-500/30 disabled:opacity-50',
  secondary:
    'inline-flex items-center gap-1.5 rounded-xl bg-white/10 px-3 py-2 text-xs font-semibold text-white/70 transition-all hover:bg-white/20 hover:text-white disabled:opacity-50'
}

function GeminiCliActionButton({
  children,
  disabled,
  icon,
  onClick,
  variant
}: GeminiCliActionButtonProps) {
  return (
    <button onClick={onClick} disabled={disabled} className={ACTION_BUTTON_CLASSNAMES[variant]}>
      {icon}
      {children}
    </button>
  )
...
function GeminiCliUsageStepCard({
  description,
  icon,
  iconClassName,
  title
}: GeminiCliUsageStepCardProps) {
  return (
    <div className="flex gap-3 rounded-xl border border-white/10 bg-white/5 p-3">
      <div className={`h-fit shrink-0 rounded-lg p-2 ${iconClassName}`}>{icon}</div>
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h4 className={`mb-1 text-sm font-bold ${authTitleClassName}`}>{authTitle}</h4>
            <p className="text-xs leading-relaxed text-white/50">{authDescription}</p>
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <GeminiCliActionButton
                ...
                variant="secondary"
                icon={
                  <RefreshIcon className={`w-3.5 h-3.5 ${isCheckingAuth ? 'animate-spin' : ''}`} />
                }
              >
                {t('gcli_check_status')}
              </GeminiCliActionButton>
        ...
        <div className="mb-6 space-y-2">
          ...
          <div className="flex items-center gap-3 rounded-xl border border-emerald-500/10 bg-emerald-500/5 p-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-bold text-white/90">{connectedAccount}</p>
              <p className="text-xs text-emerald-400/60">{t('gcli_active_session')}</p>
            </div>
          </div>
        </div>
      )}
