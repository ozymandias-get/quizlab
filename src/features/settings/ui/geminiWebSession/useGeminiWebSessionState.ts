    ...
    const handlers = useMemo<GeminiWebSessionHandlers>(() => ({
        ...
        onToggleManagedApp: (appId: GoogleWebSessionAppId) => {
            const nextEnabledAppIds = enabledAppIds.has(appId)
                ? GOOGLE_WEB_SESSION_APPS.filter((app) => app.id !== appId && enabledAppIds.has(app.id)).map((app) => app.id)
                : GOOGLE_WEB_SESSION_APPS.filter((app) => enabledAppIds.has(app.id) || app.id === appId).map((app) => app.id)
