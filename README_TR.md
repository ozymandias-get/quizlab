...
[![Sürüm](https://img.shields.io/badge/sürüm-2.0.4-blue.svg?style=flat-square)](https://github.com/ozymandias-get/quizlab/releases)
...
**Öğrenciler ve Geliştiriciler için En Kapsamlı Masaüstü Çalışma Kokpiti**  
*PDF Okuyun, Google Gemini Yapay Zeka ile Sohbet Edin ve Tek Bir Akışta Quizler Oluşturun.*

[Özellikler](#-ana-özellikler-ve-faydalar) • [Kurulum](#-hızlı-başlangıç-kılavuzu) • [Neden QuizLab?](#-neden-quizlab) • [Teknoloji](#-teknoloji-yığını)
...
Güçlü bir PDF görüntüleyiciyi bağlam farkındalığına sahip bir yapay zeka asistanıyla birleştiren **bölünmüş ekranlı bir çalışma alanı** ile çalışma oturumlarınızı yeniden tanımlayın. Sınavlara hazırlanıyor, yeni bir dil öğreniyor veya teknik belgeleri analiz ediyor olun, QuizLab uygulamadan çıkmadan **aktif hatırlama (active recall)** yapmanıza yardımcı olur.

![PDF Okuyucu ve Yapay Zeka Sohbet Arayüzünü gösteren QuizLab Uygulaması](docs/images/app-overview.png)
...
- **Özelleştirilebilir Zorluk:** Zayıf olduğunuz konuları hedeflemek için zorluk seviyelerini (Kolay, Orta, Zor), soru sayısını (1-30) ve belirli konuları ayarlayın.
- **Soru Stilleri:** Klasik, Analitik, Doğru/Yanlış veya Karışık soru stilleri arasından seçim yapın.
- **Demo Modu:** PDF yüklemeden örnek içerikle quiz oluşturmayı deneyin.
- **Oyunlaştırılmış Öğrenme:** Puanlarınızı, sürenizi ve zaman içindeki ilerlemenizi detaylı sonuçlarla takip edin.

![Çalışma Soruları Oluşturmak İçin Quiz Yapılandırma Ekranı](docs/images/quiz-creation.png)
...
- **Seçim Renkleri:** PDF metin seçimi vurgu rengini özelleştirin.
...
**S: Quiz Oluşturucu nasıl çalışır?**
C: Quiz Oluşturucu, PDF içeriğinizi analiz etmek ve bağlama uygun sorular oluşturmak için Google Gemini CLI kullanır. Zorluk, soru sayısı ve odak konuları özelleştirilebilir.

**S: Görünümü özelleştirebilir miyim?**
C: Evet! QuizLab, arka plan temaları, renkler, panel düzeni, alt çubuk opaklığı/ölçeği ve seçim renkleri dahil olmak üzere kapsamlı özelleştirme sunar.
...
- Sisteminizde **Node.js 18+** yüklü olmalıdır.
- **npm** (Node Paket Yöneticisi).
- **Google Hesabı** (Gemini CLI özellikleri için gereklidir).
- **Gemini CLI** (Quiz oluşturma için):
  ```bash
...
Uygulamayı dağıtım için derleyin:

```bash
npm run typecheck    # TypeScript türlerini kontrol et
npm run build        # Mevcut işletim sistemi için derle
npm run build:win    # Windows yükleyicisi için derle
npm run build:mac    # macOS .dmg için derle
npm run build:linux  # Linux .AppImage için derle
```
...
| Sekme | Açıklama |
|-----|-------------|
| **Promptlar** | Farklı bağlamlar için yapay zeka promptlarını özelleştirin |
| **Modeller** | Yapay zeka platformlarını etkinleştirin/devre dışı bırakın, özel yapay zeka servisleri ekleyin |
| **Gemini CLI** | Quiz oluşturma için Gemini CLI yolunu ve ayarlarını yapılandırın |
| **Seçiciler** | Otomatik yapıştırma fonksiyonu için Magic Selector'ı yapılandırın |
| **Görünüm** | Temaları, renkleri, opaklığı ve görsel tercihleri özelleştirin |
| **Dil** | İngilizce ve Türkçe arasında geçiş yapın |
| **Hakkında** | Uygulama sürümünü görüntüleyin ve güncellemeleri kontrol edin |
