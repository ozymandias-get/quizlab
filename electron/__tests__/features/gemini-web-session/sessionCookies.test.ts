...
describe('sessionCookies', () => {
  ...
  it('merges cookies by setting new ones first then removing stale Google cookies', async () => {
    ...
    const session = {
      cookies: {
        get: vi.fn().mockResolvedValue([
          ...
          { name: 'STALE_COOKIE', domain: '.google.com', path: '/old', secure: true },
    ...
    expect(remove).toHaveBeenCalledWith('https://google.com/old', 'STALE_COOKIE')
    expect(session.flushStorageData).toHaveBeenCalledTimes(1)
  })

  it('does not remove cookies that are being imported', async () => {
    const remove = vi.fn().mockResolvedValue(undefined)
    const set = vi.fn().mockResolvedValue(undefined)
    const session = {
      cookies: {
        get: vi
          .fn()
          .mockResolvedValue([{ name: 'SID', domain: '.google.com', path: '/', secure: true }]),
        remove,
        set
      },
      flushStorageData: vi.fn()
