...
vi.mock('../../../features/quiz/gemini-runner', () => ({
  ...
  executeGeminiCli: vi
    .fn()
    .mockResolvedValue([{ question: 'Test?', options: ['A', 'B'], answer: 'A' }]),
...
describe('quizCliHandler', () => {
  ...
  it('should register IPC handlers', () => {
    ...
    expect(ipcMain.handle).toHaveBeenCalledWith(
      APP_CONFIG.IPC_CHANNELS.GET_GEMINI_CLI_PATH,
      expect.any(Function)
    )
    expect(ipcMain.handle).toHaveBeenCalledWith(
      APP_CONFIG.IPC_CHANNELS.OPEN_GEMINI_LOGIN,
      expect.any(Function)
    )
    expect(ipcMain.handle).toHaveBeenCalledWith(
      APP_CONFIG.IPC_CHANNELS.CHECK_GEMINI_AUTH,
      expect.any(Function)
    )
    expect(ipcMain.handle).toHaveBeenCalledWith(
      APP_CONFIG.IPC_CHANNELS.GEMINI_LOGOUT,
      expect.any(Function)
    )
    expect(ipcMain.handle).toHaveBeenCalledWith(
      APP_CONFIG.IPC_CHANNELS.GET_QUIZ_SETTINGS,
      expect.any(Function)
    )
    expect(ipcMain.handle).toHaveBeenCalledWith(
      APP_CONFIG.IPC_CHANNELS.SAVE_QUIZ_SETTINGS,
      expect.any(Function)
    )
    expect(ipcMain.handle).toHaveBeenCalledWith(
      APP_CONFIG.IPC_CHANNELS.GENERATE_QUIZ_CLI,
      expect.any(Function)
    )
    expect(ipcMain.handle).toHaveBeenCalledWith(
      APP_CONFIG.IPC_CHANNELS.ASK_AI,
      expect.any(Function)
    )
  })
  ...
  it('GET_GEMINI_CLI_PATH handler should return valid path', async () => {
    registerQuizHandlers()
    const getCliPathCall = vi
      .mocked(ipcMain.handle)
      .mock.calls.find((call) => call[0] === APP_CONFIG.IPC_CHANNELS.GET_GEMINI_CLI_PATH)
