...
describe('systemHandlers', () => {
    ...
    it('opens external URLs only for trusted senders and valid protocols', async () => {
        ...
        await expect(openExternalHandler?.({ sender: trustedSender }, 'https://example.com')).resolves.toBe(true)
        await expect(openExternalHandler?.({ sender: trustedSender }, 'file:///secret')).resolves.toBe(false)
        await expect(openExternalHandler?.({ sender: { id: 'attacker' } }, 'https://example.com')).resolves.toBe(false)
