...
(() => {
  ...
  const hasChallengeText = url.includes('/signin/v2/challenge') || url.includes('/challenge/') || url.includes('auth_challenge') || Boolean(
    document.querySelector('[name="challengeid"], [data-challenge], form[action*="challenge"]')
  ) || [
    'verify it\\'s you',
    'verify it is you',
    '2-step verification',
  ...
  const hasSignInText = url.includes('/signin') || url.includes('/signin/') || url.includes('/servicelogin') || url.includes('signin') || Boolean(
    document.querySelector('input[type="email"], input[type="password"], [name="identifier"]')
  ) || text.includes('sign in') || text.includes('signin') || text.includes('log in') || text.includes('oturum a\\u00e7') || text.includes('giri\\u015f yap');
