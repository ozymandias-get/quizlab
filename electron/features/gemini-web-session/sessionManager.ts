...
export class GeminiWebSessionManager {
  ...
  private async performHealthCheck(options: {
    ...
    this.activeCheck = (async () => {
      ...
      try {
        const current = await this.readMetadata()
        let firstProbe = await this.runProbeAcrossApps({
          interactive: false,
          timeoutMs: HEALTH_TIMEOUT_MS
        })
  ...
  private shouldAttemptPlaywrightHeadlessRefresh(
    outcome: ProbeOutcome,
    allowRetry: boolean
  ): boolean {
    if (!allowRetry) return false
  ...
  private async runPlaywrightHeadlessRefreshProbe(
    ...
    const verificationProbe = await this.runProbeAcrossApps({
      interactive: false,
      timeoutMs: HEALTH_TIMEOUT_MS
    })
    return {
  ...
  private async runProbeAcrossApps(options: {
    interactive: boolean
    timeoutMs: number
    initialUrls?: string[]
  ...
  private async runProbe(options: {
    interactive: boolean
    timeoutMs: number
    initialUrl?: string
  }): Promise<ProbeExecutionResult> {
