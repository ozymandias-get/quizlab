...
export class ProbeRunner {
  ...
  async runProbe(options: {
    ...
    return new Promise<ProbeExecutionResult>((resolve) => {
      ...
      const handleDidFailLoad = (_event: Electron.Event, errorCode: number) => {
        if (errorCode !== -3) {
          hasNetworkError = true
        }
      }

      win.webContents.on('did-navigate', handleNavigate)
      win.webContents.on('did-redirect-navigation', handleNavigate)
