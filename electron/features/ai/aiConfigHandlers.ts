...
function sanitizeHostDescriptor(value: unknown): AutomationHostDescriptor | null {
    ...
    const nthChild = typeof raw.nthChild === 'number' && Number.isInteger(raw.nthChild) && raw.nthChild > 0
        ? raw.nthChild
        : undefined
...
function sanitizeConfig(config: unknown): AiSelectorConfig | null {
    ...
    const submitMode = raw.submitMode === undefined
        ? undefined
        : normalizeSubmitMode(raw.submitMode)

    if (raw.submitMode !== undefined && (!submitMode || String(raw.submitMode).length > MAX_SUBMIT_MODE_LENGTH)) {
        return null
    }
...
function finalizeStoredConfig(
    ...
    const canonicalHostname = canonicalizeHostname(config.canonicalHostname || sourceHostname) || sourceHostname
    const inputCandidates = config.inputCandidates && config.inputCandidates.length > 0
        ? config.inputCandidates
        : (config.input ? [config.input] : null)
    const buttonCandidates = config.buttonCandidates && config.buttonCandidates.length > 0
        ? config.buttonCandidates
        : (config.button ? [config.button] : null)
...
function mergeConfig(existing: StoredAiConfig | undefined, incoming: AiSelectorConfig): AiSelectorConfig {
...
function migrateConfigMap(map: AiConfigMap): { data: AiConfigMap; changed: boolean } {
    ...
    for (const [hostname, rawConfig] of Object.entries(map)) {
        ...
        const migrated = finalizeStoredConfig(
            normalizedHostname,
            sanitized,
            {
                defaultHealth: sanitized.version === CONFIG_VERSION ? (sanitizeConfig(rawConfig)?.health === 'ready' ? 'ready' : 'migrated') : 'migrated',
                timestamp: typeof rawConfig?.timestamp === 'number' ? rawConfig.timestamp : Date.now()
            }
        )
...
function resolveConfigForHostname(configMap: AiConfigMap, hostname?: string): StoredAiConfig | null {
    const normalizedHostname = normalizeHostname(hostname)
    if (!normalizedHostname) {
        return null
    }
    ...
    const matches = Object.values(configMap).filter((config) => config.canonicalHostname === canonicalHostname)
    if (matches.length === 1) {
...
export function registerAiConfigHandlers() {
    ...
    ipcMain.handle(IPC_CHANNELS.SAVE_AI_CONFIG, async (_event, hostname: string, config: AiSelectorConfig) => {
        ...
        const nextConfig = finalizeStoredConfig(
            normalizedHostname,
            merged,
            {
                defaultHealth: currentMap[normalizedHostname]?.version === CONFIG_VERSION
