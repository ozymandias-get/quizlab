...
export async function executeGeminiCli(
  ...
  if (prompt.length > MAX_PAYLOAD_SIZE) {
    throw new Error(
      `ValidationError: Prompt is too large (max ${MAX_PAYLOAD_SIZE} chars). Current: ${prompt.length}`
    )
  }
...
async function _executeGeminiCliInternal(
  prompt: string,
  options: ExecuteGeminiOptions
): Promise<CliResult> {
  const {
  ...
  return new Promise((resolve, reject) => {
    ...
    const childProcess = spawn(
      cliPath,
      ['-m', validateModel(model), '-p', 'Prompt:', '-o', 'json'],
      {
