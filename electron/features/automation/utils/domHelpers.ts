...
function buildCssCandidates(el: Element, kind: 'input' | 'button') {
    ...
    const isContentEditable = (el as HTMLElement).isContentEditable || el.getAttribute('contenteditable') === 'true'
    ...
    if (ariaLabel && ariaLabel.length < 256) {
        ...
        if (role) {
            pushCandidate(candidates, `[role="${CSS.escape(role)}"][aria-label="${CSS.escape(ariaLabel)}"]`)
        }
    }
    ...
    if (classTokens.length > 0) {
        pushCandidate(candidates, `${tag}${classTokens.map((token) => `.${CSS.escape(token)}`).join('')}`)
        if (role) {
            pushCandidate(candidates, `${tag}[role="${CSS.escape(role)}"]${classTokens.map((token) => `.${CSS.escape(token)}`).join('')}`)
        }
    }
...
export const generateLocatorBundle = (el: Element | null, kind: 'input' | 'button'): LocatorBundle | null => {
    if (!el) return null
    ...
    const getPrimaryAttributeLocal = (element: Element, targetKind: 'input' | 'button') => {
        const attributeCandidates = targetKind === 'input'
            ? ['data-testid', 'name', 'placeholder', 'aria-label']
            : ['data-testid', 'aria-label', 'title', 'name']
    ...
    const buildCssCandidatesLocal = (element: Element, targetKind: 'input' | 'button') => {
        ...
        const isContentEditableValue = (element as HTMLElement).isContentEditable || element.getAttribute('contenteditable') === 'true'
        ...
        if (placeholderValue && placeholderValue.length < 256) {
            pushCandidateLocal(selectorCandidates, `${tagName}[placeholder="${CSS.escape(placeholderValue)}"]`)
        }

        if (ariaLabelValue && ariaLabelValue.length < 256) {
            pushCandidateLocal(selectorCandidates, `${tagName}[aria-label="${CSS.escape(ariaLabelValue)}"]`)
            if (roleValue) {
                pushCandidateLocal(selectorCandidates, `[role="${CSS.escape(roleValue)}"][aria-label="${CSS.escape(ariaLabelValue)}"]`)
            }
        }
        ...
        if (targetKind === 'input' && isContentEditableValue) {
            ...
            if (roleValue) {
                pushCandidateLocal(selectorCandidates, `${tagName}[contenteditable="true"][role="${CSS.escape(roleValue)}"]`)
            }
        }

        if (classTokenValues.length > 0) {
            pushCandidateLocal(selectorCandidates, `${tagName}${classTokenValues.map((token) => `.${CSS.escape(token)}`).join('')}`)
            if (roleValue) {
                pushCandidateLocal(selectorCandidates, `${tagName}[role="${CSS.escape(roleValue)}"]${classTokenValues.map((token) => `.${CSS.escape(token)}`).join('')}`)
            }
        }

        return selectorCandidates.slice(0, 12)
    }
    const normalizeTextLocal = (value: string | null | undefined) => String(value || '').replace(/\s+/g, ' ').trim()
