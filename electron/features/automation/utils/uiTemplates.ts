...
export const getStepHtml = (step: PickerStep, t: TranslationMap = {}) => {
  ...
  if (step === 'input') {
    ...
    const text =
      t.picker_intro_text || 'Find the box where you type messages and <b>click on it</b>.'
  ...
  // ADIM 2: Test karakteri yaz
    ...
    const text =
      ...
      'Type <b>any letter</b> in the selected area. The send button will appear.'
...
export const getHintHtml = (
  step: PickerStep,
  hoveredInfo: PickerElementInfo | null,
  t: TranslationMap = {}
) => {
  if (!hoveredInfo || (step !== 'input' && step !== 'submit')) return ''

  const confidenceColors: Record<PickerConfidence, string> = {
    high: '#4ade80',
    medium: '#fbbf24',
    low: '#f87171'
  }
