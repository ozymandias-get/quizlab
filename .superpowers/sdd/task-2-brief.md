...
- Consumes: none (standalone JS file, independent of other tasks)
- Produces: Extension connects to bridge server and stays reconnect-capable forever

## Summary of Changes

Replace the entire `background.js` with a version that:

1. **Removes `MAX_RECONNECT_ATTEMPTS`** — no max attempt limit, reconnects forever
...
4. **Replaces `tryReconnect()`/`attemptInitialConnection()` with `scanForBridge()` + `enterScanMode()`** — simpler design
5. **Heartbeat fail → auto enter scan mode** — if `sendCookiesToApp()` fails, `connected = false` and `enterScanMode()` is called
6. **Cleaner code** — removes `saveState()`, `persistentState`, `reconnectAttempts` (no longer needed)
...
The file is plain JS. No test suite exists for it. Verify the file is syntactically valid by reading it.
